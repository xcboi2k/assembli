        ...
        try {
            ...
            if (!response.success) {
                ...
                Sentry.captureException(
                    `Failed to create category for ${user?.username ?? 'unknown'}. ${response.message}`
                )
            } else {
                resetForm()
                stopLoading()
            ...
            Sentry.captureException(
                `Failed to create category for ${user?.username ?? 'unknown'}. Service not available right now. ${error}`
            )
        }
    }
